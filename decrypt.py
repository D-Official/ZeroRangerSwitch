...
def extract_fix_number(filename):
    fix_ind = filename.find("FIX_")
    if fix_ind == -1:
        return 999
        
    num_str = ""
    for char in filename[fix_ind + 4:]:
        if not char.isdigit():
            break
        num_str += char
    if num_str == "":
...
fix_number = extract_fix_number(os.path.basename(encrypted_path))
...
with open(f"ZeroRanger Fix {fix_number} [05005E2892018000].nsp", 'wb') as file:
